module hacker::move_unlock { 
    ...
    fun get_key(): vector<u64> {
        vector[25, 11, 6, 10, 13, 25, 12, 19, 2]
    } 

    fun get_plaintext(): vector<u64> {
        vector[
             2, 14, 13, 6, 17, 0, 19, 
             20, 11, 0, 19, 8, 14, 13,
             18, 24, 14, 20, 12, 0, 
    ...
    public entry fun catch_flag(resource_object: &mut ResourceObject, ctx: &mut TxContext) {
        let key = get_key();
        let plaintext = get_plaintext();
        move_lock::movectf_unlock(plaintext, key, resource_object, ctx);
    ...
    fun test_move_unlock() {
        ...
        // check if key works
        assert!(movectf_lock(plaintext, key) == encrypted_flag, 404);
    }
